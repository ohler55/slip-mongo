# Changelog
All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [1.5.1] - unreleased
### Changed
- Updated to the mongo v2 drivers.

### Added
- Added a timeout options to most collection calls to support the mongo v2 driver.

## [1.5.0] - 2026-06-26
### Changed
- Updated Slip version to v1.5.0

## [1.4.1] - 2026-05-14
### Changed
- Updated to v1.4.1 of slip.

## [1.4.0] - 2026-04-21
### Changed
- Updated to v1.4.0 of slip.

## [1.3.1] - 2026-01-05
### Changed
- Updated to v1.3.1 of slip.

## [1.3.0] - 2026-01-05
### Changed
- Updated to v1.3.0 of slip.

## [1.2.0] - 2025-09-06
### Changed
- Updated to v1.2.0 of slip.

## [1.1.0] - 2025-06-23
### Changed
- Updated to v1.1.0 of slip.

## [1.0.1] - 2025-04-27
### Changed
- Updated to v1.0.1 of slip.

## [0.9.5] - 2024-10-11
### Changed
- Updated to v0.9.5 of slip.
